	      ...
	      <div class="modal-body">
	        ...
	        <form name="" method="post" id="editUserForm" class="form-horizontal">
	          <div class="form-group">
	           ...
	           <div class="form-group">
	            <label for="recipient-age" class="col-sm-2 control-label">价格:</label>
	            <div class="col-sm-10">
	            	<input type="text" name="price" class="form-control" id="recipient-age">
	            </div>
	          </div>
	          <div class="form-group">
	          ...
	          <div class="form-group">
	            <label for="message-intro" class="col-sm-2 control-label">描述:</label>
	            <div class="col-sm-10">
	            	<textarea class="form-control" name="description" id="message-intro" rows="6"></textarea>
	            </div>
	          </div>
	        </form>
		    		...
		    		<tbody>
		    			<c:forEach items="${projectList}" var="project">
		    				<tr>
			    				...
			    				<td>
			    					<c:choose>
			    						<c:when test="${project.sex == 1 }">
			    							男
			    						</c:when>
			    						<c:when test="${project.sex == 0 }">
			    							女
			    						</c:when>
			    						<c:otherwise>
			    							未知
			    				...
			    				<td class="td-intro">
			    					<c:choose>
			    						<c:when test="${project.description != null}">
			    							${project.description }
			    						</c:when>
			    						<c:otherwise>
			    							无
			    				...
			    				<td>
			    					<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ifHandle" data-whatever="${project.id}-edit">编辑 </button>
			    					<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#ifHandle" data-whatever="${project.id}-delete">删除 </button>
			    				</td>
			    			</tr>
		    			</c:forEach>
		    		</tbody>
...
<%@ include file="../common/footer.jsp" %>
	...
	<script src="dist/js/admin/project-handler.js"></script>
