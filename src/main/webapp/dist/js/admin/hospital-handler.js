...
var Handler = {
	...
	fIfDelete: function(event) {
		...
		if("delete" == aVal[1]) {
			...
			modal.find(".btnConfirm").bind("click", {url: 'admin/Hospital/deleteHospital.do'}, CommonHandler.fToDelete);
		} else if("edit" == aVal[1]) {
			modal.find(".modal-title").text("编辑");
			
			var oForm = modal.find('.modal-body form').eq(0);
			
			var hos = CommonHandler.fGetUserInfo(aVal[0], 'admin/Hospital/getHospitalById.do');
			
			oForm.find("input[name=id]").val(hos.id);
			oForm.find("input[name=name]").val(hos.name);
			oForm.find("input[name=address]").val(hos.address);
			
			oForm.show();
			modal.find('.modal-body p').hide();
			modal.find(".btnConfirm").bind("click", {url: 'admin/Hospital/saveHospital.do'}, CommonHandler.fToEdit);
			...
			modal.find(".btnConfirm").bind("click", {url: 'admin/Hospital/addHospital.do'}, CommonHandler.fToAdd);
		}
	},
};
