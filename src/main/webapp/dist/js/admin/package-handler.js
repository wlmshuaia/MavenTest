...
var Handler = {
	...
	fIfDelete: function(event) {
		...
		if("delete" == aVal[1]) {
			...
			modal.find(".btnConfirm").bind("click", {url: 'admin/Package/deletePackage.do'}, CommonHandler.fToDelete);
		} else if("edit" == aVal[1]) {
			modal.find(".modal-title").text("编辑");
			
			var oForm = modal.find('.modal-body form').eq(0);
			
			var hos = CommonHandler.fGetUserInfo(aVal[0], 'admin/Package/getPackageById.do');
			
			oForm.find("input[name=id]").val(hos.id);
			oForm.find("input[name=name]").val(hos.name);
			oForm.find("textarea[name=description]").val(hos.description);
			oForm.find("input[name=hid]").val(hos.hid);
			
			oForm.show();
			modal.find('.modal-body p').hide();
			modal.find(".btnConfirm").bind("click", {url: 'admin/Package/savePackage.do'}, CommonHandler.fToEdit);
			...
			modal.find(".btnConfirm").bind("click", {url: 'admin/Package/addPackage.do'}, CommonHandler.fToAdd);
		}
	},
};
